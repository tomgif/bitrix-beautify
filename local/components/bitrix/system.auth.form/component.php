...
if (!$USER->IsAuthorized()) {
	...
	if (defined('AUTH_404')) {
		...
		$arResult['AUTH_URL'] = $APPLICATION->GetCurPageParam('login=yes', array_merge($arParamsToDelete, [
			'logout_butt', 'backurl'
		]));
	}
	...
	$url = urlencode($APPLICATION->GetCurPageParam('', array_merge($arParamsToDelete, ['backurl'])));
	...
	if (CModule::IncludeModule('security')
		...
		if (Mfa\Otp::isOtpRequiredByMandatory()) {
			$arResult['ERROR_MESSAGE'] = [
				'MESSAGE' => GetMessage('system_auth_form_otp_required'),
				'TYPE' => 'ERROR'
			];
		}
	} else {
		...
		if (!CMain::IsHTTPS() && COption::GetOptionString('main', 'use_encrypted_auth', 'N') == 'Y') {
			...
			if ($arKeys = $sec->LoadKeys()) {
				$sec->SetKeys($arKeys);
				$sec->AddToForm('system_auth_form'.$arResult['RND'], ['USER_PASSWORD']);
